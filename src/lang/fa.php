<?php

/**

 * @author   yoosefap

 * @license  https://opensource.org/licenses/MIT MIT License

 */





return [
    'meta' => [
        'week_starts_on' => 'saturday',
    ],

    'zero' => 'صفر',

    'one' => 'یک',

    'two' => 'دو',

    'three' => 'سه',

    'four' => 'چهار',

    'five' => 'پنج',

    'six' => 'شش',

    'seven' => 'هفت',

    'eight' => 'هشت',

    'nine' => 'نه',

    'ten' => 'ده',

    'eleven' => 'یازده',

    'twelve' => 'دوازده',

    'thirteen' => 'سیزده',

    'fourteen' => 'چهارده',

    'fifteen' => 'پانزده',

    'sixteen' => 'شانزده',

    'seventeen' => 'هفده',

    'eighteen' => 'هجده',

    'nineteen' => 'نوزده',

    'twenty' => 'بیست',

    'thirty' => 'سی',

    'forty' => 'چهل',

    'fifty' => 'پنجاه',

    'sixty' => 'شصت',

    'seventy' => 'هفتاد',

    'eighty' => 'هشتاد',

    'ninety' => 'نود',

    'hundred' => 'صد',

    'one_hundred' => 'صد',

    'two_hundred' => 'دویست',

    'three_hundred' => 'سیصد',

    'four_hundred' => 'چهارصد',

    'five_hundred' => 'پانصد',

    'six_hundred' => 'ششصد',

    'seven_hundred' => 'هفتصد',

    'eight_hundred' => 'هشتصد',

    'nine_hundred' => 'نهصد',

    'thousand' => 'هزار',

    'million' => 'میلیون',

    'billion' => 'میلیارد',

    'trillion' => 'تریلیون',

    'quadrillion' => 'کوادریلیون',

    'quintillion' => 'کوینتیلیون',

    'between' => ' و ',

    'between.ten' => ' و ',

    'between.decimal' => ' ',

    'negative' => 'منفی',

    'point' => 'ممیز',

    'and' => 'و',

    'ordinal_suffix' => 'م',

    'ordinal_exceptions' => [

        1 => 'اول',

        2 => 'دوم',

        3 => 'سوم',

        4 => 'چهارم',

        5 => 'پنجم',

        6 => 'ششم',

        7 => 'هفتم',

        8 => 'هشتم',

        9 => 'نهم',

        10 => 'دهم',

        11 => 'یازدهم',

        12 => 'دوازدهم',

        13 => 'سیزدهم',

        14 => 'چهاردهم',

        15 => 'پانزدهم',

        16 => 'شانزدهم',

        17 => 'هفدهم',

        18 => 'هجدهم',

        19 => 'نوزدهم',

        20 => 'بیستم',

        100 => 'صدم',

    ],

    'ordinals' => [],

    'currencies' => [

        'USD' => [

            'main_singular' => 'دلار',

            'main_plural' => 'دلار',

            'sub_singular' => 'سنت',

            'sub_plural' => 'سنت',

        ],

        'EUR' => [

            'main_singular' => 'یورو',

            'main_plural' => 'یورو',

            'sub_singular' => 'سنت',

            'sub_plural' => 'سنت',

        ],

        'GBP' => [

            'main_singular' => 'پوند',

            'main_plural' => 'پوند',

            'sub_singular' => 'پنس',

            'sub_plural' => 'پنس',

        ],

        'IRR' => [

            'main_singular' => 'ریال',

            'main_plural' => 'ریال',

        ],

        'IRT' => [

            'main_singular' => 'تومان',

            'main_plural' => 'تومان',

        ],

    ],

    'units' => [

        'kg' => ['singular' => 'کیلوگرم', 'plural' => 'کیلوگرم'],

        'g' => ['singular' => 'گرم', 'plural' => 'گرم'],

        'km' => ['singular' => 'کیلومتر', 'plural' => 'کیلومتر'],

        'm' => ['singular' => 'متر', 'plural' => 'متر'],

        'cm' => ['singular' => 'سانتی‌متر', 'plural' => 'سانتی‌متر'],

        'hour' => ['singular' => 'ساعت', 'plural' => 'ساعت'],

        'minute' => ['singular' => 'دقیقه', 'plural' => 'دقیقه'],

        'second' => ['singular' => 'ثانیه', 'plural' => 'ثانیه'],

        'day' => ['singular' => 'روز', 'plural' => 'روز'],

        'week' => ['singular' => 'هفته', 'plural' => 'هفته'],

        'month' => ['singular' => 'ماه', 'plural' => 'ماه'],

        'year' => ['singular' => 'سال', 'plural' => 'سال'],

    ],

    'weekdays' => [
        'saturday' => 'شنبه',
        'sunday' => 'یکشنبه',
        'monday' => 'دوشنبه',
        'tuesday' => 'سه‌شنبه',
        'wednesday' => 'چهارشنبه',
        'thursday' => 'پنج‌شنبه',
        'friday' => 'جمعه',
    ],

];

